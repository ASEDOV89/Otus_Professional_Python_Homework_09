...
def forecast_item_sales(model, scaler, last_known_data, forecast_days, feature_columns):
    last_date = last_known_data['sale_date'].max()
    future_dates = [last_date + timedelta(days=i) for i in range(1, forecast_days + 1)]
    future_data = pd.DataFrame({'sale_date': future_dates})

    future_data['month_sin'] = np.sin(2 * np.pi * future_data['sale_date'].dt.month / 12)
    future_data['month_cos'] = np.cos(2 * np.pi * future_data['sale_date'].dt.month / 12)
    future_data['weekday_sin'] = np.sin(2 * np.pi * future_data['sale_date'].dt.weekday / 7)
    future_data['weekday_cos'] = np.cos(2 * np.pi * future_data['sale_date'].dt.weekday / 7)
...
def forecast_with_dynamic_features(db_session):
    ...
    for item in unique_items:
        ...
        forecast_days = 20
        forecast_list = forecast_item_sales(model, scaler, item_data, forecast_days, feature_columns)
